    ...
    try:
        ...
        if image_data:
            ...
            try:
                ...
                if height_float != 0.0:
                    data_lines.append((f"Н – {height_value} км", 12, True, False))
...
def create_test_new_structure_album(processed_images, file_path):
    """
    ОНОВЛЕНА ТЕСТОВА функція з таблицями БЕЗ лівого поля
    """
    try:
        doc = Document()
        
        print("=== Creating TEST Album with LEFT-ALIGNED Tables ===")
        
        # Налаштування A4 з НОВИМИ полями (ліве поле = 0!)
        section = doc.sections[0]
        set_a4_page_format(section)
        section.left_margin = Cm(mm_to_cm(ALBUM_LAYOUT['TABLE_PAGES_LEFT_MARGIN']))  # 0мм!
        ...
        print("✓ Margins set: LEFT=0mm, TOP=20mm, RIGHT=5mm, BOTTOM=5mm")
        
        # Використовуємо оновлену функцію
        create_new_structure_pages(doc, processed_images)
        
        doc.save(file_path)
        print(f"✓ TEST album with LEFT-ALIGNED tables saved: {file_path}")
        
        return True
        
    except Exception as e:
        print(f"✗ Error creating test left-aligned album: {e}")
        return False
...
class AzimuthGUI(QMainWindow):
    ...
    def create_control_panel(self, parent):
        """Ліва панель з правильними кнопками для нової структури"""
        ...
        # ГОЛОВНА кнопка для створення альбому з новою структурою
        self.create_new_structure_btn = QPushButton("Create New Structure Album")
        ...
        # Тестова кнопка для нової структури
        self.test_new_structure_btn = QPushButton("TEST New Structure")
        self.test_new_structure_btn.clicked.connect(self.create_test_new_structure_album_gui)
        self.test_new_structure_btn.setStyleSheet("""
            QPushButton {
        ...
        """)
        layout.addWidget(self.test_new_structure_btn)
        
        # Legacy кнопки (внизу)
        legacy_label = QLabel("LEGACY:")
        legacy_label.setFont(QFont("Arial", 9, QFont.Bold))
        legacy_label.setStyleSheet("color: #999; margin-top: 10px;")
        layout.addWidget(legacy_label)
        
        self.export_new_btn = QPushButton("Legacy: Create Album")
        self.export_new_btn.clicked.connect(self.create_new_word_album)
        self.export_new_btn.setStyleSheet("font-size: 10px; color: #666;")
        layout.addWidget(self.export_new_btn)
        
        self.export_add_btn = QPushButton("Legacy: Add to Album")
        self.export_add_btn.clicked.connect(self.add_to_existing_album)
        self.export_add_btn.setStyleSheet("font-size: 10px; color: #666;")
    ...
    def create_test_new_structure_album_gui(self):
        """ТЕСТОВИЙ МЕТОД для GUI - тільки нова структура"""
        if not DOCX_AVAILABLE:
            QMessageBox.warning(self, "Warning", "python-docx library not installed")
            return
        ...
        file_path, _ = QFileDialog.getSaveFileName(
            self, "Create TEST New Structure Album", "",
            "Word Documents (*.docx);;All files (*.*)"
        )
        
        if file_path:
            try:
                success = create_test_new_structure_album(self.processed_images, file_path)
                
                if success:
                    QMessageBox.information(self, "Success", 
                                        f"TEST new structure album created!\n"
                                        f"Check the new page structure:\n"
                                        f"- Top margin: 20mm\n"
                                        f"- Side/bottom margins: 5mm\n"
                                        f"- Structure per page:\n"
                                        f"  • Paragraph 10mm\n"
                                        f"  • Table 130mm (image 1)\n"
                                        f"  • Paragraph 10mm\n"
                                        f"  • Table 130mm (image 2)\n"
                                        f"  • Paragraph 10mm\n"
                                        f"Saved: {os.path.basename(file_path)}")
                    
                    self.add_result(f"✓ TEST new structure album: {os.path.basename(file_path)}")
                else:
                    QMessageBox.critical(self, "Error", "Failed to create test new structure album")
                    
            except Exception as e:
                QMessageBox.critical(self, "Error", f"Could not create test new structure album: {str(e)}")
