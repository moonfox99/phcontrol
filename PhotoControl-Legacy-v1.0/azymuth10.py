...
def get_reliable_font(font_size):
    """Отримання курсивного Arial з детальною діагностикою"""
    try:
        print(f"🔍 === FONT DIAGNOSTICS START ===")
        print(f"Required: Arial Italic, size {font_size}px")
        
        # Курсивні шляхи з детальною перевіркою
        italic_paths = [
            ...
            "/usr/share/fonts/truetype/arial/ariali.ttf",    # Linux
            "/usr/share/fonts/TTF/ariali.ttf",               # Linux
        ]
        
        # ДЕТАЛЬНА ПЕРЕВІРКА КОЖНОГО ШЛЯХУ
        for i, font_path in enumerate(italic_paths):
            print(f"🔍 Checking path {i+1}: {font_path}")
            if os.path.exists(font_path):
                try:
                    font = ImageFont.truetype(font_path, font_size)
                    print(f"✅ SUCCESS! Italic Arial loaded from: {font_path}")
                    print(f"🔍 === FONT DIAGNOSTICS END ===")
                    return font
                except Exception as load_error:
                    print(f"❌ File exists but loading failed: {load_error}")
            else:
                print(f"❌ File not found: {font_path}")
        
        print(f"⚠️ NO ITALIC ARIAL FOUND! Trying regular Arial...")
        ...
        for font_path in regular_paths:
            print(f"🔍 Checking regular: {font_path}")
            if os.path.exists(font_path):
                try:
                    font = ImageFont.truetype(font_path, font_size)
                    print(f"⚠️ FALLBACK: Regular Arial loaded from: {font_path}")
                    print(f"🔍 === FONT DIAGNOSTICS END ===")
                    return font
                except Exception as load_error:
                    print(f"❌ Regular Arial loading failed: {load_error}")
        
        print(f"❌ NO ARIAL FOUND AT ALL! Using default font")
        print(f"🔍 === FONT DIAGNOSTICS END ===")
        return ImageFont.load_default()
        
    except Exception as e:
        print(f"❌ Font loading critical error: {e}")
        return ImageFont.load_default()
...
# ЗАМІНИТИ клас TitlePageTemplateDialog в azymuth8.py
...
class TitlePageTemplateDialog(QDialog):
    ...
    def create_margins_tab(self):
        """Створення вкладки з полями (ОПТИМІЗОВАНА)"""
        widget = QWidget()
        layout = QFormLayout()
        layout.setSpacing(15)  # ЗБІЛЬШЕНИЙ простір між полями
...
class AzimuthGUI(QMainWindow):
    ...
    def create_control_panel(self, parent):
        """Ліва панель з основними кнопками (ОНОВЛЕНА ВЕРСІЯ З СУЧАСНИМИ КОЛЬОРАМИ)"""
        ...
        # ===== РОЗДІЛ: ЗАПОВНЕННЯ ДОКУМЕНТУ (ОНОВЛЕНИЙ ПОРЯДОК) =====
        ...
        # Віджет вибору дати документу (БЕЗ лейбла "Дата:")
        ...
        self.document_date_edit.setStyleSheet("""
            ...
            QCalendarWidget QSpinBox {
                ...
                font-weight: 500;
                padding: 4px 8px;
                min-width: 80px;  /* ЗБІЛЬШЕНА ШИРИНА */
                margin: 2px;
            }
            ...
            QCalendarWidget QToolButton:hover {
                background-color: #343a40;  /* ТЕМНІШЕ ПРИ НАВЕДЕННІ */
                color: white !important;
                border: 1px solid #212529;
            }
            ...
            /* СТРІЛКИ В КНОПКАХ НАВІГАЦІЇ */
            QCalendarWidget QToolButton#qt_calendar_prevmonth {
                qproperty-text: "◀";  /* ЛІВА СТРІЛКА */
            }
            QCalendarWidget QToolButton#qt_calendar_nextmonth {
                qproperty-text: "▶";  /* ПРАВА СТРІЛКА */
            }
            ...
            QCalendarWidget QAbstractItemView {
                background-color: white;
                color: #495057 !important;
                selection-background-color: #495057;  /* ТЕМНО-СІРИЙ ФОН */
                selection-color: white !important;    /* БІЛИЙ ТЕКСТ */
                font-size: 12pt;
                border: none;
            }
        ...
        # ===== ПАКЕТНА ОБРОБКА (ПЕРЕМІЩЕНО ВГОРУ) =====
        ...
        # ГОЛОВНА кнопка для створення альбому (СПЕЦІАЛЬНИЙ СТИЛЬ)
