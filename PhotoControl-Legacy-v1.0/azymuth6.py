...
def create_complete_album(processed_images, title_data, file_path):
    """Створення повного альбому з правильними полями для титульної сторінки"""
    try:
        doc = Document()
        
        print("=== Creating Complete Album with Correct Title Page ===")
        
        # 1. ТИТУЛЬНА СТОРІНКА (поля встановлюються в create_title_page)
        print("1. Creating title page...")
        create_title_page(doc, title_data)
        
        # 2. СТОРІНКА ОПИСУ (НОВА СЕКЦІЯ з розривом сторінки)
        print("2. Creating description page with separate section...")
        ...
        create_description_page_without_margins(doc)
...
def create_description_page_without_margins(doc):
    """Створення сторінки опису БЕЗ зміни полів (поля вже встановлені)"""
    try:
        print("📋 Creating description page content (margins already set)")
        ...
        print("✓ Description page created successfully (margins were pre-set)")
        
    except Exception as e:
        print(f"✗ Error creating description page content: {e}")
...
def setup_image_cell(cell, image_data, show_borders):
    """Налаштування комірки з зображенням та текстовим полем Word для РЛС"""
    try:
        # Вертикальне центрування
        set_cell_vertical_center(cell)
        ...
        if image_data:
            # Створюємо оброблене зображення (БЕЗ тексту РЛС - він буде в Word textbox)
            ...
            if processed_image:
                ...
                print(f"✓ Image added: {effective_width}mm x {effective_height}mm")
                
                # ===== ДОДАЄМО ТЕКСТОВЕ ПОЛЕ WORD ДЛЯ РЛС =====
                if 'radar_description' in image_data and image_data['radar_description']['enabled']:
                    print("📝 Adding Word textbox for radar description")
                    add_word_textbox_to_cell(cell, image_data['radar_description'])
        ...
        print(f"✓ Image cell configured with Word textbox support")
...
def create_processed_image_from_data_without_radar(image_data):
    """Створення обробленого зображення БЕЗ тексту РЛС (тільки лінія)"""
    try:
        ...
        # Позиція підкреслення номера цілі
        ...
        # Малюємо тільки лінію (БЕЗ тексту РЛС)
        draw.line([
...
def add_word_textbox_to_cell(cell, radar_data):
    """
    Додавання псевдо-textbox через оптимізовану таблицю з точним позиціонуванням
    Поєднує стабільність простої таблиці з точним позиціонуванням
    """
    try:
        print("🔧 Creating optimized textbox as positioned table...")
        
        # Формуємо текст для текстового поля
        text_lines = []
        ...
        # Номер РЛС (символ № + номер)
        number = radar_data.get('number')
        if number and number.strip():
            text_lines.append(f"№{number.strip()}")
        
        print(f"📝 Textbox content lines: {text_lines}")
        
        if not text_lines:
            print("⚠️ No content for textbox")
            return
        
        # ===== КРОК 1: ПОЗИЦІОНУЮЧИЙ ПАРАГРАФ =====
        # Додаємо "штовхаючий" параграф для опускання textbox вниз
        positioning_para = cell.add_paragraph()
        positioning_para.alignment = WD_ALIGN_PARAGRAPH.LEFT
        
        # Розраховуємо відступ для позиціонування в лівому нижньому куті
        cell_height_mm = 130          # 130мм висота комірки
        textbox_height_mm = 25.4      # 25.4мм висота textbox
        margin_bottom_mm = 1          # 1мм відступ знизу
        
        # Конвертуємо в DXA (1мм = 56.7 DXA)
        cell_height_dxa = cell_height_mm * 56.7
        textbox_height_dxa = textbox_height_mm * 56.7
        margin_bottom_dxa = margin_bottom_mm * 56.7
        
        # Відступ зверху = висота комірки - висота textbox - відступ знизу - компенсація
        compensation_dxa = 400  # Компенсація для точного позиціонування
        top_spacing_dxa = cell_height_dxa - textbox_height_dxa - margin_bottom_dxa - compensation_dxa
        
        # Встановлюємо відступ
        pPr = positioning_para._element.get_or_add_pPr()
        spacing = OxmlElement('w:spacing')
        spacing.set(qn('w:before'), str(int(max(0, top_spacing_dxa))))
        spacing.set(qn('w:after'), '0')
        spacing.set(qn('w:line'), '240')  # Мінімальна висота рядка
        spacing.set(qn('w:lineRule'), 'exact')
        pPr.append(spacing)
        
        print(f"📍 Positioning paragraph: {top_spacing_dxa:.0f} DXA top spacing")
        
        # ===== КРОК 2: ОПТИМІЗОВАНА ТАБЛИЦЯ-TEXTBOX =====
        # Створюємо мініатюрну таблицю як textbox з чистим XML
        textbox_table_xml = f'''
        <w:tbl xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">
            <w:tblPr>
                <w:tblW w:w="{int(42.9 * 56.7)}" w:type="dxa"/>
                <w:tblLayout w:type="fixed"/>
                <w:tblCellMar>
                    <w:left w:w="{int(2.5 * 56.7)}" w:type="dxa"/>
                    <w:right w:w="{int(2.5 * 56.7)}" w:type="dxa"/>
                    <w:top w:w="{int(1.3 * 56.7)}" w:type="dxa"/>
                    <w:bottom w:w="{int(1.3 * 56.7)}" w:type="dxa"/>
                </w:tblCellMar>
            </w:tblPr>
            <w:tblGrid>
                <w:gridCol w:w="{int(42.9 * 56.7)}"/>
            </w:tblGrid>
            <w:tr>
                <w:trPr>
                    <w:trHeight w:val="{int(25.4 * 56.7)}" w:hRule="exact"/>
                </w:trPr>
                <w:tc>
                    <w:tcPr>
                        <w:tcW w:w="{int(42.9 * 56.7)}" w:type="dxa"/>
                        <w:tcBorders>
                    ...
                    {create_textbox_paragraphs_xml(text_lines)}
                </w:tc>
            </w:tr>
        </w:tbl>
        '''
        
        # Парсимо XML і створюємо елемент таблиці
        textbox_table_element = parse_xml(textbox_table_xml)
        
        # Додаємо таблицю до комірки
        cell._element.append(textbox_table_element)
        
        print("✅ Optimized textbox table successfully created")
        
    except Exception as e:
        print(f"❌ Error creating optimized textbox: {e}")
        import traceback
        traceback.print_exc()
        
        # ===== FALLBACK: ПРОСТИЙ ПАРАГРАФ З РАМКОЮ =====
        try:
            print("🔄 Using simple paragraph fallback...")
            
            # Створюємо простий параграф з рамкою
            fallback_para = cell.add_paragraph()
            fallback_para.alignment = WD_ALIGN_PARAGRAPH.LEFT
            
            # Додаємо рамку до параграфа
            pPr = fallback_para._element.get_or_add_pPr()
            
            # Рамка навколо параграфа
            ...
            # Внутрішні відступи
            spacing = OxmlElement('w:spacing')
            spacing.set(qn('w:before'), '200')  # Відступ зверху
            spacing.set(qn('w:after'), '200')   # Відступ знизу
            pPr.append(spacing)
            
            ind = OxmlElement('w:ind')
            ind.set(qn('w:left'), '200')   # Лівий відступ
            ind.set(qn('w:right'), '200')  # Правий відступ
            pPr.append(ind)
            
            # Додаємо текст з правильним форматуванням
            combined_text = '\n'.join(text_lines)
            run = fallback_para.add_run(combined_text)
            run.font.name = 'Arial'
            run.font.size = Pt(12)
            run.font.italic = True
            
            print("✅ Simple paragraph fallback created")
            
        except Exception as fallback_error:
            print(f"❌ Even fallback failed: {fallback_error}")


def create_textbox_paragraphs_xml(text_lines):
    """
    Створює XML для параграфів всередині textbox таблиці
    """
    paragraphs_xml = ""
...
def calculate_dxa_positioning(cell_height_mm, textbox_height_mm, margin_bottom_mm):
    """
    Розрахунок точного позиціонування в DXA одиницях
    1мм = 56.7 DXA (Twentieths of a point)
    """
    dxa_per_mm = 56.7
    
    cell_height_dxa = cell_height_mm * dxa_per_mm
    textbox_height_dxa = textbox_height_mm * dxa_per_mm
    margin_bottom_dxa = margin_bottom_mm * dxa_per_mm
    
    # Обчислюємо відступ зверху для позиціонування в нижньому куті
    top_spacing_dxa = cell_height_dxa - textbox_height_dxa - margin_bottom_dxa
    
    return {
        'cell_height_dxa': cell_height_dxa,
        'textbox_height_dxa': textbox_height_dxa,
        'margin_bottom_dxa': margin_bottom_dxa,
        'top_spacing_dxa': max(0, top_spacing_dxa)
    }


def debug_textbox_positioning():
    """
    Функція для дебагу позиціонування textbox
    """
    print("=== Textbox Positioning Debug ===")
    
    # Параметри
    cell_height_mm = 130
    textbox_height_mm = 25.4
    margin_bottom_mm = 1
    
    # Розрахунки
    calc = calculate_dxa_positioning(cell_height_mm, textbox_height_mm, margin_bottom_mm)
    
    print(f"Cell height:     {cell_height_mm}mm = {calc['cell_height_dxa']:.0f} DXA")
    print(f"Textbox height:  {textbox_height_mm}mm = {calc['textbox_height_dxa']:.0f} DXA")
    print(f"Bottom margin:   {margin_bottom_mm}mm = {calc['margin_bottom_dxa']:.0f} DXA")
    print(f"Top spacing:     {calc['top_spacing_dxa']:.0f} DXA")
    print(f"Top spacing:     {calc['top_spacing_dxa'] / 56.7:.1f}mm")
    
    return calc

def mm_to_pt(mm):
    """Конвертує міліметри в пункти (points)"""
    return mm * 2.834645669  # 1мм = 2.834645669 pt
...
def create_processed_image_from_data(image_data):
    """Створення обробленого зображення з даними (БЕЗ тексту РЛС - він додається як Word textbox)"""
    try:
        with Image.open(image_data['image_path']) as original_image:
            if original_image.mode != 'RGB':
                if original_image.mode == 'RGBA':
...
class TitlePageTemplateDialog(QDialog):
    ...
    def create_officials_tab(self):
        """Створення вкладки з посадовими особами"""
        widget = QWidget()
        layout = QVBoxLayout()
        widget.setLayout(layout)
        ...
        <b>Приклад:</b> полковник, підполковник, майор
        """)
        info_label.setStyleSheet("""
...
class AzimuthGUI(QMainWindow):
    ...
    def create_control_panel(self, parent):
        ...
        # ===== НОВИЙ РОЗДІЛ: ТИТУЛЬНА СТОРІНКА =====
    ...
    def create_default_template(self):
        """Створення базового шаблону з ПРАВИЛЬНИМИ полями"""
        default_template = {
            'name': 'Базовий шаблон',
            'unit_name': 'А0000',
            'date': '01.01.2024',
    ...
    def get_title_page_data_from_template(self):
        """Отримання даних титульної сторінки з поточного шаблону (БЕЗ номера цілі)"""
        if not self.current_template:
            # Базові дані якщо немає шаблону - ВИПРАВЛЕНІ ПОЛЯ
            return {
        ...
        # Дані з шаблону - залишаються без змін
        return {
            'date': datetime.now(),
            ...
            'margins': self.current_template['margins']  # Беруться з шаблону
        }
    ...
    def create_batch_album_new_structure(self):
        """Створення альбому з використанням шаблону титульної сторінки (БЕЗ номера цілі)"""
        ...
        file_path, _ = QFileDialog.getSaveFileName(
            self, "Create Complete Album with Template", "",
            "Word Documents (*.docx);;All files (*.*)"
        )
        
        if file_path:
            try:
                # ===== ОТРИМУЄМО ДАНІ З ПОТОЧНОГО ШАБЛОНУ (БЕЗ номера цілі) =====
                title_data = self.get_title_page_data_from_template()
                
                print(f"📋 Using template data:")
                ...
                # ВИДАЛЕНО: print(f"   Target: {title_data['target_no']}")
                
                # Створюємо повний альбом з шаблоном
                ...
                if success:
                    ...
                    QMessageBox.information(self, "Success", 
                                        f"Complete album created with template!\n"
                    ...
                    QMessageBox.critical(self, "Error", "Failed to create album with template")
                    
            except Exception as e:
                QMessageBox.critical(self, "Error", f"Could not create album: {str(e)}")
                print(f"Error creating album with template: {e}")
                import traceback
                traceback.print_exc()
