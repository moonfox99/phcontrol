...
def create_description_page_without_margins(doc):
    """Створення сторінки опису з точними розмірами рядків"""
    try:
        print("📋 Creating description page with EXACT row heights")
        ...
        print(f"   • Table width: {sum(column_widths)}cm")
        
        print("✅ Description page created with EXACT row heights + Arial 14pt!")
...
class AzimuthGUI(QMainWindow):
    ...
    def get_title_page_data_from_template(self):
        """Отримання даних титульної сторінки з оновленою логікою дати"""
        if not self.current_template:
            # Базові дані якщо немає шаблону
            return {
                'date': self.get_document_date_for_title_page(),  # ОНОВЛЕНО
                'unit_info': 'А0000',
                'commander_info': {
                    'rank': 'полковник',
                    'name': 'П.П. ПЕТРЕНКО'
