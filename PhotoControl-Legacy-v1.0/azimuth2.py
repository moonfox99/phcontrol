...
ALBUM_LAYOUT = {
    ...
    # СПРОЩЕНІ ПОЛЯ ДЛЯ СТОРІНОК З ТАБЛИЦЯМИ
    'TABLE_PAGES_LEFT_MARGIN': 0,      # 0мм зліва
    'TABLE_PAGES_RIGHT_MARGIN': 0,     # 0мм справа  
    'TABLE_PAGES_TOP_MARGIN': 25,      # 25мм зверху (замість 0!)
    'TABLE_PAGES_BOTTOM_MARGIN': 5,    # 5мм знизу (замість 0!)
    
    # Відстані для таблиць (тепер простіші)
    'RIGHT_MARGIN_FROM_TABLE': 5,      # 5мм від таблиці до правого краю
    'SPACING_BETWEEN_TABLES': 5,       # 5мм між таблицями на одній сторінці
    
    # Розміри комірок таблиці (без змін)
    'LEFT_CELL_WIDTH': 30,
    'RIGHT_CELL_WIDTH': 30,
    'RIGHT_CELL_TOP_PADDING': 15,
    
    # Параметри розташування (без змін)
    'TABLES_PER_PAGE': 2,
...
def create_complete_album(processed_images, title_data, file_path):
    try:
        ...
        print("=== Creating Simplified Album ===")
        ...
        # 3. СТОРІНКИ З ТАБЛИЦЯМИ (спрощені поля)
        print("3. Creating table pages with simplified margins...")
        create_table_pages(doc, processed_images)
        
        doc.save(file_path)
        print(f"✓ Simplified album saved: {file_path}")
        
        return True
        
    except Exception as e:
        print(f"✗ Error creating simplified album: {e}")
        return False
...
def set_standard_margins(doc):
    ...
    try:
        ...
        print(f"✓ Standard margins set on A4: {ALBUM_LAYOUT['STANDARD_LEFT_MARGIN']}mm all around")
        
    except Exception as e:
        print(f"✗ Error setting standard margins: {e}")

def create_table_pages(doc, processed_images):
        """Створення сторінок з таблицями з нульовими полями на A4"""
        try:
            ...
            # СПРОЩЕНІ ПОЛЯ: 25мм зверху, 5мм знизу, 0мм по боках
            table_section.left_margin = Cm(mm_to_cm(ALBUM_LAYOUT['TABLE_PAGES_LEFT_MARGIN']))     # 0мм
            table_section.right_margin = Cm(mm_to_cm(ALBUM_LAYOUT['TABLE_PAGES_RIGHT_MARGIN']))   # 0мм
            table_section.top_margin = Cm(mm_to_cm(ALBUM_LAYOUT['TABLE_PAGES_TOP_MARGIN']))       # 25мм
            table_section.bottom_margin = Cm(mm_to_cm(ALBUM_LAYOUT['TABLE_PAGES_BOTTOM_MARGIN'])) # 5мм
            
            print(f"✓ Simplified margins set on A4: top={ALBUM_LAYOUT['TABLE_PAGES_TOP_MARGIN']}mm, bottom={ALBUM_LAYOUT['TABLE_PAGES_BOTTOM_MARGIN']}mm")
            
            # Додаємо таблиці БЕЗ додаткових відступів
...
def add_image_tables_to_section(doc, processed_images):
    ...
    # Обробляємо зображення парами
    for i in range(0, len(processed_images), 2):
        # Перше зображення (обов'язкове)
        first_image = processed_images[i]
        
        # Друге зображення (може не бути для останньої пари)
        second_image = processed_images[i + 1] if i + 1 < len(processed_images) else None
        
        print(f"\n=== Creating unified table for images {i+1}-{i+2 if second_image else i+1} ===")
        
        # Створюємо єдину таблицю з двома зображеннями
        table = create_unified_image_table(doc, first_image, second_image)
        
        if table:
            tables_created = 2 if second_image else 1
            total_tables += tables_created
            print(f"✅ Created unified table with {tables_created} images")
        
        # Розрив сторінки після кожної єдиної таблиці (крім останньої)
        if i + 2 < len(processed_images):
            doc.add_page_break()
            print(f"📄 Page break after unified table")
    
    print(f"\n✅ Final result: {total_tables} images in {(len(processed_images) + 1) // 2} unified tables")

def create_unified_image_table(doc, first_image_data, second_image_data=None):
    """
    Створення ЄДИНОЇ таблиці з 3 рядками:
    - Рядок 1: Перше зображення (13см)
    - Рядок 2: Прозорий розділювач (5мм)
    - Рядок 3: Друге зображення або порожній (13см)
    """
    try:
        print(f"🔨 Creating unified table...")
        
        # Розрахунок ширин (без змін)
        total_page_width = ALBUM_LAYOUT['PAGE_WIDTH']  # 210мм
        right_margin = ALBUM_LAYOUT['RIGHT_MARGIN_FROM_TABLE']  # 5мм
        ...
        print(f"📏 Table width: {total_table_width}mm")
        
        # Створюємо таблицю з 3 рядками
        ...
        row1 = table.rows[0]
        row1.height = Cm(13)
        setup_image_row(row1, first_image_data, left_cell_width, middle_cell_width, right_cell_width, 13)
        print(f"✅ Row 1: First image configured")
        
        # РЯДОК 2: Прозорий розділювач (5мм)
        row2 = table.rows[1]
        row2.height = Cm(mm_to_cm(5))  # 5мм висоти
        setup_spacer_row(row2, left_cell_width, middle_cell_width, right_cell_width)
        ...
        if second_image_data:
            setup_image_row(row3, second_image_data, left_cell_width, middle_cell_width, right_cell_width, 13)
            print(f"✅ Row 3: Second image configured")
        else:
            setup_empty_row(row3, left_cell_width, middle_cell_width, right_cell_width)
            print(f"✅ Row 3: Empty row configured")
...
def setup_image_row(row, image_data, left_width, middle_width, right_width, height_cm):
    """
    Налаштування рядка з зображенням
    """
    try:
        ...
        # Встановлюємо ширини комірок
        set_absolute_cell_width(left_cell, left_width)
        set_absolute_cell_width(middle_cell, middle_width)
        set_absolute_cell_width(right_cell, right_width)
        
        # Налаштовуємо вміст комірок
        setup_left_cell(left_cell)
        setup_middle_cell(middle_cell, image_data, middle_width, height_cm)
        
        # Для правої комірки без додаткового відступу зверху (row вже має свою висоту)
        setup_right_cell_in_row(right_cell, image_data)
        
        # Рамки: всі комірки мають всі рамки
        for cell in [left_cell, middle_cell, right_cell]:
            set_cell_borders(cell, top=True, bottom=True, left=True, right=True)
        
        # Тільки ліва комірка має спеціальні рамки (без лівої)
        set_cell_borders(left_cell, top=True, bottom=True, left=False, right=True)
        set_cell_borders(right_cell, top=True, bottom=True, left=False, right=True)
        
    except Exception as e:
        print(f"❌ Error setting up image row: {e}")

def setup_spacer_row(row, left_width, middle_width, right_width):
    """
    Налаштування прозорого розділового рядка (5мм)
    """
    try:
        left_cell = row.cells[0]
        middle_cell = row.cells[1]
        ...
        # СПЕЦІАЛЬНІ РАМКИ: тільки верх і низ для візуального розділення
        for cell in [left_cell, middle_cell, right_cell]:
            set_cell_borders(cell, top=True, bottom=True, left=False, right=False)
        
        # Тільки права комірка має праву рамку
        set_cell_borders(right_cell, top=True, bottom=True, left=False, right=True)
        
        print(f"📏 Spacer row: transparent with top/bottom borders only")
        
    except Exception as e:
        print(f"❌ Error setting up spacer row: {e}")

def setup_empty_row(row, left_width, middle_width, right_width):
    """
    Налаштування порожнього рядка (коли немає другого зображення)
    """
    try:
        left_cell = row.cells[0]
        middle_cell = row.cells[1]
        ...
        # Порожній вміст з рамками
        for cell in [left_cell, middle_cell, right_cell]:
            cell.text = ""
            set_cell_background(cell, "FFFFFF")
            set_cell_borders(cell, top=True, bottom=True, left=True, right=True)
        
        # Спеціальні рамки для лівої та правої комірок
        set_cell_borders(left_cell, top=True, bottom=True, left=False, right=True)
        set_cell_borders(right_cell, top=True, bottom=True, left=False, right=True)
        
        print(f"📄 Empty row configured")
        
    except Exception as e:
        print(f"❌ Error setting up empty row: {e}")

def setup_right_cell_in_row(cell, image_data):
    """
    Налаштування правої комірки БЕЗ додаткового відступу зверху
    (використовується в рядках єдиної таблиці)
    """
    try:
        # Очищуємо комірку
        cell.text = ""
        
        # Перший параграф БЕЗ відступу зверху (висота рядка вже задана)
        para = cell.paragraphs[0]
        para.alignment = WD_ALIGN_PARAGRAPH.LEFT
        para.paragraph_format.space_before = Pt(0)  # БЕЗ відступу зверху
        ...
        print("✅ Right cell configured for table row")
        
    except Exception as e:
        print(f"✗ Error configuring right cell in row: {e}")

def create_table_pages_with_zero_margins(doc, processed_images):
    """
    ОНОВЛЕНА функція для роботи з єдиними таблицями
    """
    try:
        if not processed_images:
            print("No processed images to add")
        ...
        # Поля: 25мм зверху, 5мм знизу, 0мм по боках
        table_section.left_margin = Cm(0)
        table_section.right_margin = Cm(0)
        table_section.top_margin = Cm(mm_to_cm(ALBUM_LAYOUT['TABLE_PAGES_TOP_MARGIN']))       # 25мм
        table_section.bottom_margin = Cm(mm_to_cm(ALBUM_LAYOUT['TABLE_PAGES_BOTTOM_MARGIN'])) # 5мм
        
        print(f"✅ Page margins set: top={ALBUM_LAYOUT['TABLE_PAGES_TOP_MARGIN']}mm, bottom={ALBUM_LAYOUT['TABLE_PAGES_BOTTOM_MARGIN']}mm")
        
        # Додаємо єдині таблиці
        add_image_tables_to_section(doc, processed_images)
        
    except Exception as e:
        print(f"✗ Error creating table pages: {e}")

def create_precise_image_table(doc, image_data, target_no, is_second_on_page=False):
    ...
    try:
        ...
        if is_second_on_page:
            # ДРУГА ТАБЛИЦЯ: вирівнювання до нижнього поля
            print(f"→ SECOND table - aligning to bottom margin")
            
            # Розраховуємо скільки потрібно відступу зверху щоб друга таблиця була внизу
            page_height = ALBUM_LAYOUT['PAGE_HEIGHT']                    # 297мм
            top_margin = ALBUM_LAYOUT['TABLE_PAGES_TOP_MARGIN']          # 25мм  
            bottom_margin = ALBUM_LAYOUT['TABLE_PAGES_BOTTOM_MARGIN']    # 5мм
            table_height_mm = 130  # мм
            
            # Доступний простір між полями
            available_height = page_height - top_margin - bottom_margin  # 297 - 25 - 5 = 267мм
            
            # Відступ для вирівнювання другої таблиці до низу
            spacing_to_bottom = available_height - table_height_mm - table_height_mm  # 267 - 130 - 130 = 7мм
            
            spacer = doc.add_paragraph()
            spacer.paragraph_format.space_before = Cm(mm_to_cm(spacing_to_bottom))
            
            print(f"→ Calculated spacing to bottom: {spacing_to_bottom}mm")
        else:
            # ПЕРША ТАБЛИЦЯ: впритул до верхнього поля
            print(f"→ FIRST table - aligned to top margin")
        
        # Створення таблиці (без змін)
        ...
        print(f"✓ Created simplified table for target {target_no}")
        return table
...
def setup_middle_cell(cell, image_data, cell_width_mm, cell_height_cm):
    ...
    try:
        ...
        # Створюємо оброблене зображення з ВИРІВНЯНОЮ лінією
        ...
        if processed_image:
            ...
            print(f"✓ Image with aligned line: {image_width_cm:.1f}x{image_height_cm:.1f}cm")
        ...
        print("✓ Middle cell configured with aligned line")
...
def setup_right_cell(cell, image_data, target_no):
    ...
    try:
        ...
        # Перший параграф з відступом 30pt зверху (замість 15мм)
        para = cell.paragraphs[0]
        para.alignment = WD_ALIGN_PARAGRAPH.LEFT
        
        # ВИПРАВЛЕННЯ 1: 30pt замість 15мм
        para.paragraph_format.space_before = Pt(30)  # 30pt замість Cm(1.5)
        para.paragraph_format.space_after = Pt(0)    # Без відступу після
        
        # ВИПРАВЛЕННЯ 2: Міжрядковий інтервал 1.15
        para.paragraph_format.line_spacing = 1.15    # Множинний 1.15
        ...
        # Дані для відображення
        data_lines = [
            (f"{target_no}", 12, True, True),                                    # Номер цілі
            (f"β – {analysis_point['azimuth']:.0f}°", 12, True, False),         # Азимут
            (f"D – {analysis_point['range']:.1f} км", 12, True, False),         # Дальність
            ("без перешкод", 9, True, False),                                   # Перешкоди  
            (f"{target_data['detection']}", 9, True, False),                    # Виявлення
            (f"М – {target_data['scale']}", 9, True, False)                     # Масштаб
        ]
        
        # Додаємо рядки
        for i, (text, font_size, italic, underline) in enumerate(data_lines):
            if i > 0:
                # Новий параграф для кожного рядка (крім першого)
                para = cell.add_paragraph()
                para.alignment = WD_ALIGN_PARAGRAPH.LEFT
                
                # ВИПРАВЛЕННЯ 3: Міжрядковий інтервал для всіх параграфів
                para.paragraph_format.line_spacing = 1.15
                para.paragraph_format.space_before = Pt(0)  # Без відступу зверху
                para.paragraph_format.space_after = Pt(0)   # Без відступу знизу
        ...
        print("✓ Right cell configured with fixed formatting")
...
def create_processed_image_from_data(image_data):
    try:
        ...
        print(f"Error creating aligned line image: {e}")
        return None
...
def create_tables_only_album(processed_images, file_path):
    """
    ТЕСТОВА функція - створення тільки сторінок з таблицями
    Без титульної та описової сторінок
    """
    try:
        doc = Document()
        
        print("=== Creating FIXED Tables Only Album ===")
        
        # Налаштування A4 з нульовими полями
        section = doc.sections[0]
        set_a4_page_format(section)
        ...
        print("✓ Zero margins set on A4")
        
        # Використовуємо ВИПРАВЛЕНУ функцію
        add_image_tables_to_section(doc, processed_images)
        
        doc.save(file_path)
        print(f"✓ FIXED tables-only album saved: {file_path}")
        
        return True
        
    except Exception as e:
        print(f"✗ Error creating fixed album: {e}")
        return False
...
class AzimuthGUI(QMainWindow):
    ...
    def create_control_panel(self, parent):
        ...
        """
        Додайте цей код в create_control_panel після існуючих кнопок
        """
        # Тестова кнопка
        test_separator = QFrame()
        test_separator.setFrameShape(QFrame.HLine)
        test_separator.setFrameShadow(QFrame.Sunken)
        test_separator.setStyleSheet("color: #ccc; margin: 10px 0px;")
        layout.addWidget(test_separator)
        ...
        self.test_tables_btn = QPushButton("Create TEST Album (Tables Only)")
        self.test_tables_btn.clicked.connect(self.create_test_batch_album)
        self.test_tables_btn.setStyleSheet("""
            QPushButton {
        ...
        """)
        layout.addWidget(self.test_tables_btn)
        
        layout.addStretch()
        parent.addWidget(control_widget)
    
    def create_test_batch_album(self):
        """
        ТЕСТОВИЙ МЕТОД для класу AzimuthGUI - тільки таблиці
        Додайте цей метод в клас AzimuthGUI
        """
        ...
        file_path, _ = QFileDialog.getSaveFileName(
            self, "Create FIXED TEST Album", "",
            "Word Documents (*.docx);;All files (*.*)"
        )
        
        if file_path:
            try:
                success = create_tables_only_album(self.processed_images, file_path)
                
                if success:
                    QMessageBox.information(self, "Success", 
                                        f"FIXED TEST album created!\n"
                                        f"Check console output for spacing logic\n"
                                        f"Should show:\n"
                                        f"- Page 1: Table 1 (25mm), Table 2 (5mm)\n"
                                        f"- Page 2: Table 3 (25mm), Table 4 (5mm)\n"
                                        f"Saved: {os.path.basename(file_path)}")
                    
                    self.add_result(f"✓ FIXED TEST album: {os.path.basename(file_path)}")
                else:
                    QMessageBox.critical(self, "Error", "Failed to create fixed test album")
                    
            except Exception as e:
                QMessageBox.critical(self, "Error", f"Could not create fixed test album: {str(e)}")
