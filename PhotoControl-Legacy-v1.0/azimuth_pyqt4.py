...
class AzimuthGUI(QMainWindow):
    def __init__(self):
        ...
        # НОВІ змінні для збереження налаштувань сітки між зображеннями
        self.saved_grid_settings = {
            'center_offset_x': 0,  # Зміщення центру відносно центру зображення
            'center_offset_y': 0,
            'scale_edge_point': None,
    ...
    def create_control_panel(self, parent):
        """Оновлена ліва панель з поміненими місцями розділами і покращеними стилями"""
        ...
        # ПЕРЕНЕСЕНИЙ БЛОК: Batch processing section (тепер зверху)
        ...
        # ПЕРЕНЕСЕНИЙ БЛОК: Results area (тепер знизу)
        ...
        self.results_text.setMaximumHeight(200)  # Зменшено висоту
    ...
    def create_report_panel(self, parent):
        """Права панель БЕЗ центральної кнопки в навігації"""
        ...
        # ОНОВЛЕНА навігація БЕЗ центральної кнопки
        move_layout = QGridLayout()
        move_layout.setSpacing(4)  # Трохи більший проміжок для кращого вигляду
        ...
        # Кнопки тепер розташовані хрестом БЕЗ центральної
        btn_up = QPushButton("↑")
        ...
        # ЦЕНТРАЛЬНОЇ КНОПКИ БІЛЬШЕ НЕМАЄ!
    ...
    def move_center_and_save(self, dx, dy):
        ...
        # Перерахувати поточну точку аналізу якщо існує
    ...
    def load_folder_thumbnails(self):
        ...
        self.current_folder_images = image_files  # Ініціалізація для batch processing
    ...
    def load_image(self, file_path):
        ...
        try:
            ...
            if self.has_saved_grid_settings():
                self.add_result("Applied saved grid settings")
    ...
    def apply_saved_grid_settings(self):
        ...
        # Відновлюємо scale edge point якщо він був збережений
        if self.saved_grid_settings['scale_edge_point'] and self.saved_grid_settings['custom_scale_distance']:
            # Перераховуємо координати scale edge відносно нового центру
            saved_point = self.saved_grid_settings['scale_edge_point']
            
            # Відносні координати від центру
            rel_x = saved_point['x'] - (self.processor.image.width // 2)
            rel_y = saved_point['y'] - (self.processor.image.height // 2)
            
            # Нові абсолютні координати
            new_x = self.processor.center_x + rel_x
            new_y = self.processor.center_y + rel_y
    ...
    def save_current_grid_settings(self):
        ...
        self.saved_grid_settings = {
            ...
            'scale_edge_point': self.scale_edge_point.copy() if self.scale_edge_point else None,
    ...
    def has_saved_grid_settings(self):
        ...
        return (self.saved_grid_settings['center_offset_x'] != 0 or 
                ...
                self.saved_grid_settings['scale_edge_point'] is not None or
    ...
    ef set_scale_edge_point(self, x, y):
